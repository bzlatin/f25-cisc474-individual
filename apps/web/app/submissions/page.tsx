...
export default function SubmissionsPage() {
  ...
  const [ensParams, setEnsParams] = useState(''); // e.g., dataset=small,seed=42
  const [submitting, setSubmitting] = useState(false);
  const [ok, setOk] = useState(false);
  const [err, setErr] = useState<string | null>(null);

  // Basic validation message for the active submission type
  ...
  useEffect(() => {
    // optional: if no assignment provided, redirect back to assignments
  ...
  const onSubmit = async () => {
    try {
      ...
      // For demo: log the payload
      // You would POST to /api/submissions here.
      // console.log({ assignmentId, type, file, url, repo, ensFile, ensParams });
          ...
          {type === 'ens' && (
            <Stack spacing={1.5}>
              ...
              <Alert severity="info" variant="outlined">
                ENS submissions run in a sandbox with course-defined tests.
                You’ll receive structured feedback and artifacts after
                execution.
              </Alert>
            </Stack>
          )}
